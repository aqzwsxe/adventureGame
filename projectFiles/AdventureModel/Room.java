...
public class Room implements Serializable {
    ...
    public String getObjectString() {
        ...
        for (int i = 0; i < this.objectsInRoom.size(); i++) {
            AdventureObject obj = this.objectsInRoom.get(i);
            joiner.add(obj.getDescription());
        }
    ...
    public String getCommands() {
        ArrayList<String> dir_list = new ArrayList<>();
        StringJoiner joiner = new StringJoiner(", ");
        final List<Passage> passageTable = this.motionTable.getDirection();
        for (int i = 0; i < passageTable.size(); i++) {
            Passage passage = passageTable.get(i);
            final String direction = passage.getDirection();
            if (!dir_list.contains(direction)) {
                dir_list.add(direction);
                joiner.add(direction);
            }
        }
        return joiner.toString(); //replace this!
    }
