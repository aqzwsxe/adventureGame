...
public class AdventureGame implements Serializable {
    ...
    private final HashMap<String,String> synonyms; //A HashMap to store synonyms of commands.
    private final String[] actionVerbs = {"QUIT","INVENTORY","TAKE","DROP","RESTART"}; //List of action verbs (other than motions) that exist in all games. Motion vary depending on the room and game.
    ...
    public boolean movePlayer(String direction) {
        ...
        for (Passage entry : possibilities) {
            ...
            if (chosen == null && entry.getIsBlocked()) {
                containKey = iterator.matchKey(this.player.getInventory(), entry.getKeyName());

                if (containKey) {

                    System.out.println("the contain key is true, move from: "+ this.player.getCurrentRoom().getRoomNumber());
        ...
        previousRoom = this.player.getCurrentRoom();
    ...
    public String interpretAction(String command){
        ...
        if (motionTable.optionExists(inputArray[0])) {
            ...
            if(inputArray[0].equals("RESTART")) { this.resetGame(); return null; } //Reset game
     ...
     *
     * @implNote Invalidates all previously associated game objects
     */
